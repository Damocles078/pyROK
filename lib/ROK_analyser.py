...
pytesseract.pytesseract.tesseract_cmd =r'C:\\Program Files\\Tesseract-OCR\\tesseract.exe' #set path to tesseract.exe

class ROKanalyzer():

    custom_config_names: ClassVar[str] = r'-l chi_sim+eng+rus+jpn+kor --oem 3 --psm 7' #setting considered languages for nicknames
    ...
    def __init__(self,
        ...
        self.time: str = str(datetime.datetime.now().strftime("%Y_%m_%d__%H_%M_%S"))
        self.request: str = request
        self.worker_thread: threading.Thread = threading.Thread(target=self.worker).start()
    ...
    def worker(self) -> None:
        ...
        while(not self._stop_event.is_set() and not self.gui.close.is_set()):
            if(not self.queue.qsize() == 0):
                ...
                id = self.acquire_ID_from_image(image1[250:300, 925:1095])
                try:
                    if self.acquire_name:
                        name = self.acquire_name_from_img(image2[129:180, 467:880])
                    else:
                        name = "Not Acquired"
                except:
                    name = "failed"
                ...
                try:
                    if self.acquire_power:
                        power = self.acquire_power_from_img(image2[120:190, 980:1195])
                    else:
                        power = -1
                except:
                    power = -2
                ...
                try:
                    if self.acquire_alliance:
                        alliance = self.acquire_alliance_from_img(image1[398:450,780:900])
                    else:
                ...
                try:
                    if self.acquire_T4:
                        t4 = self.acquire_T4_from_img(image1[838:875, 1500:1710])
                    else:
                        t4 = -1
                except:
                    t4 = -2
                ...
                try:
                    if self.acquire_T5:
                        t5 = self.acquire_T5_from_img(image1[890:925, 1500:1710])
                    else:
                        t5 = -1
                except:
                    t5 = -2
                ...
                try:
                    if self.acquire_dead:
                        dead = self.acquire_dead_from_img(image2[505:550, 1260:1580])
                    else:
                        dead = -1
                except:
                    dead = -2
                ...
                try:
                    if self.acquire_rssAssist:
                        rssAssist = self.acquire_rssAssist_from_img(image2[780:830, 1260:1580])
                    else:
                        rssAssist = -1
                except:
                    rssAssist = -2
                ...
                with connection:
                    ...
                    mycursor.execute(self.request, (self.time, id, name, alliance, power, t4, t5, dead, rssAssist))
                    connection.commit()
    ...
    def acquire_ID_from_image(self, image: cv2.Mat) -> int:
        ...
        text: str = pytesseract.image_to_string(cleaned_image, config=self.custom_config_IDs)
    ...
    def acquire_name_from_img(self, image: cv2.Mat) -> str:
        cleaned_image = traitement(image)
        text: str = pytesseract.image_to_string(cleaned_image, config=self.custom_config_names) #identify text from image using more lannguages than default
        res = text.split()
        fin = ''
        try:
            fin = res[0]
    ...
    def acquire_alliance_from_img(self, image: cv2.Mat) -> str:
        cleaned_image = traitement(image)
        text: str = pytesseract.image_to_string(cleaned_image, config=self.custom_config_alliances)
    ...
    def acquire_power_from_img(self, image: cv2.Mat) -> int:
        cleaned_image = traitement(image)
        text: str = pytesseract.image_to_string(cleaned_image, config=self.custom_config_numbers)
        text = text.replace(',','').replace('.','').replace(':','')
        text = text.split()
        res = ''
        for t in text:
