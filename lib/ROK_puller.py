...
import win32api
import win32con  # import input controller
...
class ROKpuller():
    ...
    def Opening(self, a):  # tries to open a player stats and return if actually opened
        img1 = np.asarray(ImageGrab.grab(bbox=(int(200*self.x_coef), int(600*self.y_coef),
                          int(1600*self.x_coef), int(1000*self.y_coef))))  # capture before click screen
        self.leftClick()
        time.sleep(a)
        img2 = np.asarray(ImageGrab.grab(bbox=(int(200*self.x_coef), int(600*self.y_coef),
                          int(1600*self.x_coef), int(1000*self.y_coef))))  # capture after click screen
        # compares before/after click image to determine if stats are open
        opened = self.compare_images(img1, img2)
        time.sleep(0.1)
        return opened
