...
class ROKpuller():
    ...
    def dragClick(self):
        ...
        time.sleep(.01)
        for k in range(23):
            self.mousePos((int(860*self.x_coef),int((700-k*5)*self.y_coef)))
            time.sleep(.05)
        time.sleep(0.3)
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP,0,0)
    ...
    def screenGrab(self) -> str: #setup command to screenshot
        time.sleep(.1)
        im = ImageGrab.grab(bbox = (0,int(29*self.y_coef),int(1920*self.x_coef),int(1040*self.x_coef))) #capture only game 
...
## open kills
    ...
    def openKills(self): #open details of kills
        self.mousePos((int(1340*self.x_coef),int(420*self.y_coef)))
...
## open dead
    ...
    def openDead(self): # open dead data
        self.mousePos((int(455*self.x_coef),int(800*self.y_coef)))
        self.Opening(0.3)
        time.sleep(0.1)
...
## return to leaderboard
    ...
    def returnLeaderBoard(self): #from dead data, return to leaderboard page
        self.mousePos((int(1670*self.x_coef),int(60*self.y_coef)))
        self.leftClick()
        time.sleep(1)
        self.mousePos((int(1635*self.x_coef),int(120*self.y_coef)))
        self.leftClick()
        time.sleep(1)
...
## Acquire first 4
    ...
    def first4(self): # record the 4 first players
        time.sleep(0.1)
        print((int(860*self.x_coef),int(270*self.y_coef)))
        self.mousePos((int(860*self.x_coef),int(335*self.y_coef)))
        if self.Opening(1.5):
            self.AcquirePlayer()
            self.count += 1
            self.GUI.updateBar()
        ...
        self.mousePos((int(860*self.x_coef),int(460*self.y_coef)))
        if self.Opening(1.5):
            self.AcquirePlayer()
            self.count += 1
            self.GUI.updateBar()
            ...
            self.GUI.updateBar(inc=2)
        self.mousePos((int(860*self.x_coef),int(575*self.y_coef)))
        if self.Opening(1.5):
            self.AcquirePlayer()
            self.count += 1
            self.GUI.updateBar()
            ...
            self.GUI.updateBar(inc=2)
        self.mousePos((int(860*self.x_coef),int(700*self.y_coef)))
        if self.Opening(1.5):
            self.AcquirePlayer()
            self.count += 1
            self.GUI.updateBar()
...
## Acquire players
    ...
    def Acquire(self): # record top x
        ...
        for k in range(self.to_capture-4): # record others
            if(k<994):
                self.mousePos((int(860*self.x_coef),int(725*self.y_coef)))
                if keyboard.is_pressed("p") or self.GUI.close.is_set():
                    stop = True
                    break
                if self.Opening(1):
                    self.AcquirePlayer()
        ...
        if not stop and self.to_capture == 1000 and not self.GUI.close.is_set():
            self.mousePos((int(860*self.x_coef),int(848*self.y_coef)))
            if self.Opening(1.5):
                self.AcquirePlayer()
                self.count += 1
                self.GUI.updateBar()
                ...
                self.GUI.updateBar(inc=2)
            self.mousePos((int(860*self.x_coef),int(968*self.y_coef)))
            if self.Opening(1.5):
                self.AcquirePlayer()
                self.count += 1
                self.GUI.updateBar()
